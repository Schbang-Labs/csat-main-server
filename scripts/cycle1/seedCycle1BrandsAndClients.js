/**
 * Seed Script - Cycle 1 Brands and Clients
 *
 * This script:
 * 1. Checks if brands exist in DB, if not creates them with isActive: false and empty services
 * 2. Seeds all brand entries in brandHistory with brandId and cycleId 1
 * 3. Checks if clients exist in DB, if not creates them with isActive: false
 * 4. Seeds all clients in clientHistory with cycleId 1
 *
 * Run with: node scripts/cycle1/seedCycle1BrandsAndClients.js
 */
...
/**
 * Cycle 1 Brand Data from brand_service_segregation_cycle1.md
...
async function seedBrands(cycleId) {
  ...
  for (const entry of CYCLE1_DATA) {
  ...
  for (const [brandName, brandData] of brandMap) {
    try {
      ...
      // Build services array for history (with actual services from Cycle 1 data)
      const services = Array.from(brandData.services).map(serviceName => ({
        department: DEPT_MAP[serviceName],
        isActive: true,
      }));

      // Create/update brand history for Cycle 1
  ...
  console.log(
    `✅ Brand History: ${historyCreated} entries created/updated for Cycle 1`
  );
...
async function seedClients(cycleId, _brandMap) {
  ...
  for (const entry of CYCLE1_DATA) {
    try {
      ...
      // Create/update client history for Cycle 1
  ...
  console.log(
    `✅ Client History: ${historyCreated} entries created/updated for Cycle 1`
  );
}
 ...
 */
async function seed() {
  console.log('🌱 Starting Cycle 1 Seeding...\n');
  ...
  try {
    ...
    console.log(`   Brand History (Cycle 1): ${brandHistoryCount}`);
    console.log(`   Client History (Cycle 1): ${clientHistoryCount}`);

    console.log('\n🎉 Cycle 1 seeding completed successfully!');
  } catch (error) {
