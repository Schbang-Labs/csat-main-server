/**
 * Seed Script - Cycle 2 SBUs and SBU History (All 7 Departments)
 *
 * This script:
 ...
 * 5. Create SBU History entries for ALL SBUs (existing + new) for Cycle 2
 *
 * Data Sources:
 * - SBU_lineUp_.csv: Solutions (Lines 1-282), Media (Lines 283-341), Tech (Lines 342-378)
 * - SBU_LIneUp_remaining.csv: SEO (Lines 1-49), MarTech (Lines 50-82), Fluence (Lines 83-120), SMP (Lines 121-160)
 *
 * Departments: Solutions, Media, Tech, SEO, MarTech, Fluence, SMP
 *
 * Run with: node scripts/cycle2/seedCycle2SBUs.js
 */
...
/**
 * SBU Data from SBU_lineUp_.csv - SOLUTIONS Department (Lines 2-282)
 * Lead combinations with their associated brands for Cycle 2
 */
const CYCLE2_SOLUTIONS_SBU_LEADS = [
  {
...
/**
 * SBU Data from SBU_lineUp_.csv - MEDIA Department (Lines 284-341)
 */
const CYCLE2_MEDIA_SBU_LEADS = [
  {
    leads: ['Mrugank Desai'],
    brands: [
      "Papa Don't Preach",
...
/**
 * SBU Data from SBU_lineUp_.csv - TECH Department (Lines 343-378)
 */
const CYCLE2_TECH_SBU_LEADS = [
  {
...
/**
 * SBU Data from SBU_LIneUp_remaining.csv - SEO Department (Lines 2-49)
 */
const CYCLE2_SEO_SBU_LEADS = [
  {
    leads: ['Akshay Chatlani'],
    brands: [
      'Bridgestone',
...
/**
 * SBU Data from SBU_LIneUp_remaining.csv - MARTECH Department (Lines 51-82)
 */
const CYCLE2_MARTECH_SBU_LEADS = [
  {
    leads: ['Akshay Chatlani'],
...
/**
 * SBU Data from SBU_LIneUp_remaining.csv - FLUENCE Department (Lines 84-120)
 */
const CYCLE2_FLUENCE_SBU_LEADS = [
  {
    leads: ['Sneh Chedda'],
    brands: [
...
/**
 * SBU Data from SBU_LIneUp_remaining.csv - SMP Department (Lines 122-160)
 */
const CYCLE2_SMP_SBU_LEADS = [
  {
...
async function getOrCreateCycle2() {
  ...
  if (!cycle) {
    cycle = await Cycle.create({
      ...
      endDate: new Date(2025, 4, 31, 23, 59, 59, 999), // May 31
      status: 'completed',
      isActive: false,
    });
 ...
 */
async function seed() {
  console.log('🌱 Starting Cycle 2 SBU Seeding (All 7 Departments)...\n');
  ...
  try {
    ...
    console.log(`   Total SBU History entries for Cycle 2: ${sbuHistoryCount}`);

    console.log(
      '\n🎉 Cycle 2 SBU seeding completed successfully for ALL 7 departments!'
    );
  } catch (error) {
