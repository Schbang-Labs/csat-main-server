/**
 * Seed Script - Cycle 2 Brands and Clients
 *
 * This script:
 * 1. Checks if brands exist in DB, if not creates them with isActive: false and empty services
 * 2. Seeds all brand entries in brandHistory with brandId and cycleId 2
 * 3. Checks if clients exist in DB, if not creates them with isActive: false
 * 4. Seeds all clients in clientHistory with cycleId 2
 *
 * Run with: node scripts/cycle2/seedCycle2BrandsAndClients.js
 */
...
/**
 * Cycle 2 Brand Data from brand_service_segregation_cycle2.md
...
async function seedBrands(cycleId) {
  ...
  for (const entry of CYCLE2_DATA) {
  ...
  for (const [brandName, brandData] of brandMap) {
    try {
      ...
      // Build services array for history (with actual services from Cycle 2 data)
      const services = Array.from(brandData.services).map(serviceName => ({
        department: DEPT_MAP[serviceName],
        isActive: true,
      }));

      // Create/update brand history for Cycle 2
  ...
  console.log(
    `✅ Brand History: ${historyCreated} entries created/updated for Cycle 2`
  );
...
async function seedClients(cycleId, _brandMap) {
  ...
  for (const entry of CYCLE2_DATA) {
    try {
      ...
      // Create/update client history for Cycle 2
  ...
  console.log(
    `✅ Client History: ${historyCreated} entries created/updated for Cycle 2`
  );
}
 ...
 */
async function seed() {
  console.log('🌱 Starting Cycle 2 Seeding...\n');
  ...
  try {
    ...
    console.log(`   Brand History (Cycle 2): ${brandHistoryCount}`);
    console.log(`   Client History (Cycle 2): ${clientHistoryCount}`);

    console.log('\n🎉 Cycle 2 seeding completed successfully!');
  } catch (error) {
